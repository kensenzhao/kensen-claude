...
## 怎么用这个插件       — 只列**本插件自己**的能力(技能/命令/脚本),最常用的置顶
##                       安装在上方小节、更新链回根 README,这里不再重列通用命令
...
- **跨文件链接别依赖 emoji 标题的锚点**(`#-保持更新` 这类 CJK+emoji 锚点不稳),链到文件本身即可。
- **同一信息只有一个家**(SSOT):重复就改成链接,别两处各写一份(否则必然漂移——这正是本插件要消灭的东西)。

### ✅ 发布前 README 自检表
- [ ] 插件 README 顶部有一句话定位 + 回链根 README。
- [ ] "怎么用"一节只列本插件能力,最常用置顶;无通用 `/plugin` 命令(除安装 3 行)。
- [ ] 没有与根 README 重复的市场介绍/更新说明(改成链接)。
- [ ] 深内容(原理/边界/手动用法)在 `<details>` 折叠,披露不超过 2 层。
- [ ] 根 README 的"收录插件"表已加该插件一行;无单插件 `##` 段。
- [ ] 所有跨文件链接可点(不赌 emoji 锚点)。
